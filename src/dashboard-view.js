...
function digestCampaignDirectory(directory) {
  try {
    ...
    return { campaignId: stream.campaignId, units: [...byUnit.values()], message: null };
  } catch (error) {
    return {
...
export function renderDashboardContent(snapshot) {
  ...
  return `${message}<nav class="view-tabs" aria-label="Dashboard views">`
    ...
    + '<button type="button" data-view="detail" aria-pressed="false">Detail</button></nav>'
...
const state={snapshot:JSON.parse(document.getElementById('initial-dashboard-data').textContent),view:'triage',threshold:2,attentionOnly:true,detailRunId:null};
...
stream.addEventListener('snapshot',function(event){state.snapshot=JSON.parse(event.data).snapshot;renderSessions();renderLive();syncDetailOptions();if(state.view==='detail')refreshDetail();connection.textContent='Live';refreshAges()});
