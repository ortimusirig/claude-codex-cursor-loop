...
export function renderDashboardContent(snapshot) {
  ...
  return `${message}<nav class="view-tabs" aria-label="Dashboard views">`
    ...
    + '<button type="button" data-view="logs" aria-pressed="false">Logs</button></nav>'
    + renderTriage(snapshot) + renderLive(snapshot) + renderDetail(snapshot) + renderLogs(snapshot);
}
...
const state={snapshot:JSON.parse(document.getElementById('initial-dashboard-data').textContent),view:'triage',threshold:2,attentionOnly:true,detailRunId:null,logsRunId:'all',logsProblemsOnly:false};
