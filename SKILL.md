...
- **Outcomes:** `review-ready`, `no-op`, `gate-failed`, `verifier-failed` when either Cursor pass exits non-zero without producing a result or assistant event, or `timed-out` when a terminal stage exceeds its deadline.
- **Exit codes:** `0` on review-ready or no-op, `1` on gate-failed, `2` on preflight/arg failure, `3` on an unexpected fatal error or unrecognised outcome, `4` on verifier-failed, `5` on timed-out.
